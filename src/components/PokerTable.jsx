...
const PlayerAvatar = ({ player }) => {
  if (player.isBot) {
    if (player.id === 'bot1') {
      return (
        <motion.div 
          className="w-16 h-16 mx-auto mb-2 rounded-full overflow-hidden border-2 border-[#9370DB] shadow-lg shadow-[#9370DB]/20"
  ...
  return (
    <motion.div 
      className="w-16 h-16 mx-auto mb-2 rounded-full bg-gradient-to-br from-[#1E1E2E] to-[#2D2D44] flex items-center justify-center text-2xl border-2 border-[#9370DB] shadow-lg shadow-[#9370DB]/20"
      whileHover={{ rotate: 360 }}
      transition={{ duration: 1 }}
    >
      {player.avatar}
...
const PokerTable = ({ 
  ...
  const getPlayerPositions = () => {
    ...
    // Position bots in specific corners
    ...
    // Human player at bottom middle
    if (humanPlayer) {
      positions[humanPlayer.id] = {
        position: 'absolute',
        left: '45%',
        bottom: `${margin}px`,
        transform: 'translate(-50%, 0)'
    ...
    <div className="relative w-full h-screen bg-[#0A0A0F] overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-4">
        ...
        >
          <img 
            ...
            className="w-12 h-12"
          />
          <div className="text-[#9370DB] font-bold text-xl">
            ${pot}
          </div>
        </motion.div>
      </div>
      ...
      {currentPlayer && (
        <div className="absolute bottom-4 right-4 flex flex-row gap-2">
          <motion.button 
            className="bg-gradient-to-r from-red-600 to-red-700 text-white font-bold py-2 px-6 rounded-xl text-sm shadow-lg shadow-red-600/20"
          ...
          <motion.button 
            className="bg-gradient-to-r from-[#9370DB] to-[#8A2BE2] text-white font-bold py-2 px-6 rounded-xl text-sm shadow-lg shadow-[#9370DB]/20"
          ...
          <motion.button 
            className="bg-gradient-to-r from-[#8A2BE2] to-[#9370DB] text-white font-bold py-2 px-6 rounded-xl text-sm shadow-lg shadow-[#8A2BE2]/20"
      ...
      <div className="absolute inset-0">
        {players.map((player) => (
          <motion.div
            key={player.id}
            className={`absolute ${player.isBot ? 'w-32' : 'w-40'}`}
          ...
          >
            <motion.div 
              className={`bg-black/30 backdrop-blur-sm rounded-xl p-3 border border-[#9370DB]/20 ${
                player.id === currentPlayer?.id ? 'ring-2 ring-[#9370DB] shadow-lg shadow-[#9370DB]/20' : ''
              }`}
            ...
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="text-center">
                  <div className="font-medium text-white/90 text-sm mb-1">{player.name}</div>
                  <motion.div 
                    className="text-[#9370DB] font-bold text-sm"
                ...
                <div className="flex justify-center gap-1 mt-2">
                  {player.cards.map((card, cardIndex) => (
                    <motion.div
                      key={cardIndex}
                      className="transform scale-50 origin-center shadow-xl cursor-pointer"
                      initial={{ rotateY: 0 }}
                      animate={{ rotateY: player.id === currentPlayer?.id ? (cardsSeen ? 0 : 180) : 180 }}
                      transition={{ duration: 0.6 }}
                      onClick={handleSeeCards}
                    >
                      <Card
                        card={card}
                        isFaceUp={player.id === currentPlayer?.id ? cardsSeen : false}
                      />
                    </motion.div>
                  ))}
                </div>
                ...
                {/* Dealer Button */}
                {players.indexOf(player) === dealerPosition && (
                  <motion.div 
